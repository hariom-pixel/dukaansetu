...
} from "@/services/accounting-db.service";
...
export default function Accounting() {
  ...
  const [confirmDel, setConfirmDel] = useState<JournalEntryRow | null>(null);

  async function loadAccounting() {
    const [journalRows, summaryRows] = await Promise.all([
      getJournalEntries(),
      getAccountingSummary(),
    ]);

    setItems(journalRows);
    setSummary(summaryRows);
  }

  useEffect(() => {
    loadAccounting().catch(console.error);

    const reloadAccounting = () => {
      loadAccounting().catch(console.error);
    };

    window.addEventListener("focus", reloadAccounting);
    window.addEventListener("visibilitychange", reloadAccounting);

    return () => {
      window.removeEventListener("focus", reloadAccounting);
      window.removeEventListener("visibilitychange", reloadAccounting);
    };
  }, []);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteJournalEntry(String(confirmDel.id));
      await loadAccounting();
  ...
  const cashPos = ledger.length ? ledger[ledger.length - 1].balance : summary.cashPosition;
        ...
        actions={
          <>
            <Button
              ...
              size='sm'
              className='gap-1.5'
              onClick={() => toast.success('GST report generated')}
            >
              <Download className='h-4 w-4' /> GST report
            </Button>
            <Button
              size='sm'
        ...
        <Card className='lg:col-span-2 shadow-soft border-border/60 overflow-hidden'>
          <div className='p-5 border-b border-border'>
            ...
            <p className='text-xs text-muted-foreground'>
              Every entry traceable to source document
            </p>
          </div>
          ...
          <Card className='p-5 shadow-soft border-border/60'>
            ...
            <div className='space-y-2'>
              <Button
                ...
                onClick={() => toast.success('P&L generated')}
              >
                <FileText className='h-4 w-4' /> Generate P&L
              </Button>
              <Button
                variant='outline'
                className='w-full justify-start gap-2 text-sm'
                onClick={() => toast.success('Balance sheet generated')}
              >
              ...
              <Button
                variant='outline'
                className='w-full justify-start gap-2 text-sm'
                onClick={() => toast.success('Bank reconciliation started')}
              >
                <Receipt className='h-4 w-4' /> Bank reconcile
              </Button>
              <Button
                variant='outline'
                className='w-full justify-start gap-2 text-sm'
                onClick={() => toast.success('Tally export queued')}
              >
                <Download className='h-4 w-4' /> Export Tally
              </Button>
            </div>
          </Card>
        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Add a debit or credit to the cash ledger.
            </DialogDescription>
          </DialogHeader>
          <div className='grid gap-3 py-2'>
            ...
            <div className='grid gap-1.5'>
              ...
              <Input
                ...
                placeholder='e.g. Cash deposit · HDFC'
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button className='bg-gradient-primary' onClick={submit}>
              Add entry
            </Button>
          </DialogFooter>
        </DialogContent>
